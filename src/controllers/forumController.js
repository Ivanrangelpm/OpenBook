...
function listarComentarios(req, res) {
    ...
    forumModel.listarComentarios().then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
...
function cadastrarComentario(req, res) {
    ...
    var fkTopico = req.body.fkTopicoServer;
    var fkUsuario = req.body.fkUsuarioServer;

    // Faça as validações dos valores
    if (comentario == undefined) {
        res.status(400).send("Seu comentario está undefined!");
    } else if (fkTopico == undefined) {
        res.status(400).send("Sua fkTopico está undefined!");
    } else if (fkUsuario == undefined) {
        res.status(400).send("Sua fkUsuario está undefined!");
    } else {

        // Passe os valores como parâmetro e vá para o arquivo usuarioModel.js
        forumModel.cadastrarComentario(comentario, fkTopico, fkUsuario)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
                ...
                function (erro) {
                    console.log(erro);
                    console.log(
                        "\n ja estou em forumController !Houve um erro ao realizar o cadastro do comentario! Erro: ",
                        erro.sqlMessage
                    );
                    res.status(500).json(erro.sqlMessage);
                }
